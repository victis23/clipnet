...
struct SampleData {
	func loadSampleCampaignData() -> [Campaign] {
		[
			Campaign(
	...
	func loadSampleClipData() -> [ClipSubmission] {
		let campaignData = loadSampleCampaignData()
		let caleb = campaignData[0]
		let aiden = campaignData[3]
		let pirate = campaignData[1]
